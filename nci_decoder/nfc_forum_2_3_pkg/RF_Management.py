...
def RF_FIELD_INFO_NTF(raw, vendor="None", model="None"):
	...
	print("  * RF Field Status: "+NFC_table.tbl_rf_field_status.get(rf_field_status, NFC_table.tbl_rf_field_status['00']))
	p_payload = p_payload + 2*1
	# print("")	
	return p_payload
	...
	for i in range(n):
		...
		if((type_val == "00") | (type_val == "01")):
			...
			if((nfcee_id_i >= 2) & (nfcee_id_i <= 15)):
				...
				nfcee_id = '80-FE'
			print("      * NFCEE:", data_val[0:2], "("+NFC_table.tbl_nfcee_id.get(nfcee_id,"RFU")+")")
...
def RF_SET_FORCED_NFCEE_ROUTING_CMD(raw, vendor="None", model="None"):
	...
	if(forced_nfcee_rout_state == "00"):
		...
		if((int(forced_nfcee,16) >= 2) & (int(forced_nfcee,16) <= 15)):
			...
			route_val = '80-FE'
		print("  * Forced NFCEE: "+forced_nfcee+" ("+NFC_table.tbl_nfcee_id.get(route_val,"RFU")+")")
		p_payload = p_payload + 2*1
...
# new ctrl msg
# 21 12
def RF_REMOVAL_DETECTION_CMD(raw, vendor="None", model="None"):
	"""""""""""""""
		[RF_REMOVAL_DETECTION_CMD]
    WAITING_TIME:				1 Octet
	"""""""""""""""
	# NFC_table = pkg_import.tbl_import(vendor, model)
	p_payload = 0

	wt = raw[p_payload:(p_payload+2*1)]
	print("  * WAITING_TIME:", wt) # p.207 有點看不懂文件
	p_payload = p_payload + 2*1
...
# 41 12
def RF_REMOVAL_DETECTION_RSP(raw, vendor="None", model="None"):
	"""""""""""""""
		[RF_REMOVAL_DETECTION_RSP]
    Status:		1 Octet
	"""""""""""""""
	# NFC_table = pkg_import.tbl_import(vendor, model)
	p_payload = 0
...
# 61 12
def RF_REMOVAL_DETECTION_NTF(raw, vendor="None", model="None"):
	"""""""""""""""
		[RF_REMOVAL_DETECTION_NTF]
    Status:		1 Octet
	"""""""""""""""
	# NFC_table = pkg_import.tbl_import(vendor, model)
	p_payload = 0
...
# 61 13
def RF_WLC_STATUS_NTF(raw, vendor="None", model="None"):
	"""""""""""""""
		[RF_WLC_STATUS_NTF]
    WLC Status Condition:				1 Octet
	Length of WPT duration:				1 Octet
	WPT duration:						0-1 Octet
	Length of Transmit Power level:		1 Octet
	Transmit Power level:				0-1 Octet
	Length of Battery level:			1 Octet
	Battery level:						0-1 Octet
	"""""""""""""""
	# NFC_table = pkg_import.tbl_import(vendor, model)
	p_payload = 0

	wlc_status = raw[p_payload:(p_payload+2*1)]
	if(wlc_status == '00'):
		print("  * WLC Status Condition:", "Static WPT mode Start")
	elif(wlc_status == '01'):
		print("  * WLC Status Condition:", "Negotiated WPT mode Start")
	elif(wlc_status == '02'):
		print("  * WLC Status Condition:", "Communication phase start")
	elif(wlc_status == '03'):
		print("  * WLC Status Condition:", "NEGO wait error")
	elif(wlc_status == '04'):
		print("  * WLC Status Condition:", "NEGO wait – timer started")
	elif(wlc_status == '05'):
		print("  * WLC Status Condition:", "Charging phase start")
	elif(wlc_status == '06'):
		print("  * WLC Status Condition:", "Stop Pattern detected")
	elif(wlc_status == '07'):
		print("  * WLC Status Condition:", "FO detected")
	elif(wlc_status == '08'):
		print("  * WLC Status Condition:", "Unspecified error")
	elif(wlc_status == '09'):
		print("  * WLC Status Condition:", "Remote NFC Endpoint non-responsive")
	elif(int(wlc_status, 16) >= 10 and int(wlc_status, 16) <= 127):
		print("  * WLC Status Condition:", "RFU", "("+wlc_status+")")
	elif(int(wlc_status, 16) >= 128 and int(wlc_status, 16) <= 255):
		print("  * WLC Status Condition:", "Proprietary", "("+wlc_status+")")
	p_payload = p_payload + 2*1

	l = int(raw[p_payload:(p_payload+2*1)], 16)
	p_payload = p_payload + 2*1

	if(l != 0):
		wpt_dura = raw[p_payload:(p_payload+2*l)]
		wpt_dura_b = bin(int(wpt_dura, 16))[2::].zfill(8)
		print("  * WPT duration:", int(wpt_dura_b[3:], 2), "("+wpt_dura+")")
		p_payload = p_payload + 2*l
	...
	if(l != 0):
		tx_pwr_lv = raw[p_payload:(p_payload+2*l)]
		print("  * Tx Power level::", str(int(tx_pwr_lv, 16))+"%", "("+tx_pwr_lv+")")
		p_payload = p_payload + 2*l
	...
	if(l != 0):
		battery_lv = raw[p_payload:(p_payload+2*l)]
		print("  * Battery level::", str(int(battery_lv, 16))+"%", "("+battery_lv+")")
		p_payload = p_payload + 2*l
	# print("")
	return p_payload

# 21 14
def GET_WLCP_INFO_PARAM_CMD(raw, vendor="None", model="None"):
	"""""""""""""""
		[GET_WLCP_INFO_PARAM_CMD]
	Number of Parameters: 	1 Octet (n)
	...
	num_of_param = raw[p_payload:(p_payload+2*1)]
	n = int(num_of_param, 16)
	print("  * Number of Params:", n)
	p_payload = p_payload + 2*1
	...
	for i in range(n):
		...
		print("    * ID "+str(i)+":", end=" ")
		print(NFC_table.tbl_wlc_poll_tlv.get(para_id,"RFU ("+para_id+")"))
		p_payload = p_payload + 2*1
	# print("")
	return p_payload

# 41 14
def GET_WLCP_INFO_PARAM_RSP(raw, vendor="None", model="None"):
	"""""""""""""""
		[GET_WLCP_INFO_PARAM_RSP]
	Status: 				1 Octet
	Number of Parameters: 	1 Octet (n)
	...
	for i in range(n):
		print("   ~ [Param_"+str(i)+"] ~  ")
		para_id = raw[p_payload:(p_payload+2*1)]
		print("    * ID: "+NFC_table.tbl_wlc_poll_tlv.get(para_id,"RFU ("+para_id+")"))
		p_payload = p_payload + 2*1
...
# 21 15
def WPT_START_CMD(raw, vendor="None", model="None"):
	"""""""""""""""
		[WPT_START_CMD]
	Number of Parameters: 	1 Octet (n)
	Parameter [1..n]: 		m+2 Octets
	...
	for i in range(n):
		print("   ~ [Param_"+str(i)+"] ~  ")
		para_id = raw[p_payload:(p_payload+2*1)]
		print("    * ID: "+NFC_table.tbl_wpt_start_cmd_tlv.get(para_id,"RFU ("+para_id+")"))
		p_payload = p_payload + 2*1
...
# 41 15
def WPT_START_RSP(raw, vendor="None", model="None"):
	"""""""""""""""
		[WPT_START_RSP]
    Status:		1 Octet
	"""""""""""""""
...
# 61 15
def WPT_START_NTF(raw, vendor="None", model="None"):
	"""""""""""""""
		[WPT_START_NTF]
    WPT End Condition:		1 Octet
	"""""""""""""""
	# NFC_table = pkg_import.tbl_import(vendor, model)
	p_payload = 0
	
	wpt_end = raw[p_payload:(p_payload+2*1)]
	if(wpt_end == '00'):
		print("  * WLC Status Condition:", "Time completed")
	elif(wpt_end == '01'):
		print("  * WLC Status Condition:", "FOD")
	elif(wpt_end == '02'):
		print("  * WLC Status Condition:", "Stop Pattern")
	elif(wpt_end == '03'):
		print("  * WLC Status Condition:", "Unspecified error")
	elif(int(wpt_end, 16) >= 4 and int(wpt_end, 16) <= 127):
		print("  * WLC Status Condition:", "RFU", "("+wpt_end+")")
	elif(int(wpt_end, 16) >= 128 and int(wpt_end, 16) <= 255):
		print("  * WLC Status Condition:", "Proprietary", "("+wpt_end+")")
	p_payload = p_payload + 2*1
	# print("")
	return p_payload