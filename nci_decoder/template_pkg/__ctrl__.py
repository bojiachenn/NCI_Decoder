...
from template_pkg import RF_Management
from template_pkg import NFCEE_Management
from template_pkg import Proprietary
...
tbl_nci_ctrl = {
	...
	# A new vendor rarely needs proprietary messages on day one - this section
	# is here so Decoder_Main.py's GID lookup ("Proprietary") has somewhere to
	# resolve to; add entries here (see Nxp_pkg/__ctrl__.py's "Proprietary"
	# section for the real, currently-dispatched shape) as the chip's actual
	# proprietary command set is documented.
	"Proprietary": {
	},
}
